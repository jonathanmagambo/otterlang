...
C_TIMES=()
for i in {1..5}; do
    TIME=$(/usr/bin/time -p ./pi_leibniz_c >/dev/null 2>&1 | grep real | awk '{print $2}')
    C_TIMES+=($TIME)
    echo "  Run $i: ${TIME}s"
done
...
RUST_TIMES=()
for i in {1..5}; do
    TIME=$(/usr/bin/time -p ./pi_leibniz_rust >/dev/null 2>&1 | grep real | awk '{print $2}')
    RUST_TIMES+=($TIME)
    echo "  Run $i: ${TIME}s"
done
...
OTTER_TIMES=()
for i in {1..5}; do
    TIME=$(/usr/bin/time -p ./pi_leibniz_otter >/dev/null 2>&1 | grep real | awk '{print $2}')
    OTTER_TIMES+=($TIME)
    echo "  Run $i: ${TIME}s"
done
